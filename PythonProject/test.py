...
class SpImgThread(QThread):
    ...
    def __init__(self):
        ...
        self.MODE = 0
        # self.photo_sensor = 17                  # 光电传感器所连GPIO脚
        # GPIO.setmode(GPIO.BCM)                  # BCM编码
        # GPIO.setup(self.photo_sensor, GPIO.IN)  # GPIO0输入模式
        # self.LED_Pin = 26                       # LED所连GPIO脚
        # GPIO.setup(self.LED_Pin, GPIO.OUT)      # GPIO0输出模式
    ...
    def screenWakeUp(self):
        # os.system('xset dpms force on')   # 唤醒
        # os.system('xset dpms 0 0 0 ')     # 防止休眠
        # os.system('xset s off')
        pass

    # 关闭屏幕
    def setScreenOff(self):
        # os.system('xset dpms force off')  # 立即关闭屏幕
        pass

    # 状态机
    def stateCheck(self):
        if self.speech.robotText:            # 机器人有回话
            self.communicateSignal.emit(self.robotText)
        ...
        if self.MODE == 0:                   # 等待状态
            ...
            if not self.face.isFaceVideoRun():# 如果FaceVideo不在运行
                ...
                # self.face.faces_video()
            if self.face.getHappyFace:        # 若已获得happy图片
                print(">> 停止人脸检测")
